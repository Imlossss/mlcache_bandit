...
static int arms;
static int cnt[MAX_ARMS];
static long arms_scores[MAX_ARMS];
#endif

#ifdef CONFIG_MLCACHE_ACTIVE

static void update_arms_score(bool hit)
{
...
{
	if (arms == 0)
		p->mlcache_score = -p->lru_mlcache;
	if (arms == 1)
		p->mlcache_score = p->lru_mlcache;
	if (arms == 2)
		p->mlcache_score = -p->fifo;
	/* TODO UCB etc */
}
static void update_page(struct page *page, struct address_space *mapping)
{
	page->lru_mlcache = t;
	if (page->f != -1){ 
		page->fifo = t;
	...
	// 遍历整个 address_space 的 page_tree
	radix_tree_for_each_slot(slot, &mapping->page_tree, &iter, 0)
	{
		...
		page_mlscores_change(p);
	}

	rcu_read_unlock();

}
static void update_arm(pgoff_t index, struct page *page, struct address_space *mapping, bool hit)
{
	update_arms_score(hit);
	update_page(page, mapping);
}
...
#ifdef CONFIG_MLCACHE_ACTIVE
		t++;
		update_arm(off, page, mapping, hit);
		if (t <= MAX_BLOCK * MAX_ARMS)
		{
			arms = (t - 1) / MAX_ARMS;
			cnt[arms] = 1;
		}
		else if (t % MAX_BLOCK == 0)
		{
			arms = 0;
			int i;
			for (i = 1; i < MAX_ARMS; i++)
			{
				if (arms_scores[arms] < arms_scores[i])
				{
					arms = i;
				}
			}
